...
//The api has some fields that are different than the field keys
const fieldMap = {
  member_count: "attenders",
  believer_count: "believers",
  baptized_count: "baptized",
  baptized_in_group_count: "newlyBaptized",
};

/**
 * Register a church health count to show above the circle
 * @param {*} key
 * @returns
 */
const addMetric = (key) => {
  field = group_fields[key];
  if (!showMetrics || !field || field.hidden) {
    return;
  }

  //Add the icon SVG element
  metricIcons[key + "_icon"] = {
    type: "image",
    attributes: {
      y: -2.5 * textHeight,
  ...
  //Add the number SVG element
  metricNumbers.push({
    header: fieldMap[key] ?? key,
    svg: {
      type: "text",
      attributes: {
        y: -0.5 * textMargin,
  ...
  /**
   * Position the metrics icons and numbers
   */
  let indent =
    Object.entries(metricIcons).length > 0
      ? ((4 - Object.entries(metricIcons).length) * countSpacing) / 2
      : 0;

  Object.entries(metricIcons).forEach(([key], index) => {
    metricIcons[key].attributes.x = indent + countLeft + countSpacing * index;
  });

  metricNumbers.forEach((metric, index) => {
    metric.svg.attributes.x = indent + countNumberLeft + countSpacing * index;
  });
};

addMetric("member_count");
addMetric("believer_count");
addMetric("baptized_count");
addMetric("baptized_in_group_count");
